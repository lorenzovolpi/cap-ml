        ...
        try:
            for k in number_successes:
                ...
                assert k <= self.number_trials, (
                    'Values in input list must be smaller or equal to the number of input probabilities "n"'
                )
        except TypeError:
            assert type(number_successes) == int or type(number_successes) == np.int64, (
                "Input value must be an integer."
            )
            assert number_successes >= 0, "Input value cannot be negative."
            assert number_successes <= self.number_trials, "Input value cannot be greater than " + str(
                self.number_trials
            )
        return True
